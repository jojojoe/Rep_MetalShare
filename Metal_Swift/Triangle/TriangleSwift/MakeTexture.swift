...
//    let scaledImage = UIImage.scaleToSize(mirrorImage, size: size)
    ...
    do {
        ...
        let _:[String: NSNumber] = [ MTKTextureLoader.Option.SRGB.rawValue: false]
...
//         textureLoader.newTexture(cgImage: image.cgImage!, options: [:])
        return try textureLoader.newTexture(with: image.cgImage!, options: MTKTextureLoader.Option)
    }
}
